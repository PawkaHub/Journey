...
- (void)connection:(NSURLConnection*)connection didReceiveResponse:(NSURLResponse*)response
{
	...
	UnityReportWWWReceivedResponse(self.udata, self->_status, (unsigned)self->_estimatedLength, [self->_responseHeader UTF8String]);
}
...
- (void)connection:(NSURLConnection*)connection didReceiveData:(NSData*)data
{
	...
	UnityReportWWWReceivedData(self.udata, (unsigned)[self->_data length], (unsigned)self->_estimatedLength);
...
{
	UnityReportWWWSentData(self.udata, (unsigned)totalBytesWritten, (unsigned)totalBytesExpectedToWrite);
}
...
{
	return (int)[((UnityWWWConnectionDelegate*)connection).data length];
}
